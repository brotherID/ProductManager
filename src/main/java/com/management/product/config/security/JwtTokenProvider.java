...
public class JwtTokenProvider {
    ...
    public String generateToken(Authentication authentication) {
        ...
        return Jwts.builder()
                ...
                .signWith(key(), SignatureAlgorithm.HS256) // Utilise `key()` pour la clé secrète
                .compact();
    }
    ...
    public String getUsername(String token) {
        ...
        return claims.getSubject(); // Récupère le `subject` (username)
    }
    ...
    public boolean validateToken(String token) {
        try {
            Jwts.parserBuilder()
                    .setSigningKey(key()) // Utilise la clé secrète pour valider le token
                    .build()
                    .parseClaimsJws(token); // Analyse et valide le JWT
